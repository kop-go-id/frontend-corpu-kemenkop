...
/**
 * Maps payload to standardized structure with body and param
 * @param {Object} payload - Input payload
 * @param {Object} [payload.body] - Request body data
 * @param {Object} [payload.params] - Request parameters
 * @returns {{body: any, params: any}} Standardized payload object with body and param
 */
export const mapPayload = (payload, { paramSnakeCase = false } = {}) => {
  if (payload.body || payload.params) {
    if (paramSnakeCase) {
      return {
        body: payload.body,
        params: Object.keys(payload.params).reduce((acc, key) => {
          acc[snakeCase(key)] = payload.params[key];
          return acc;
        }, {}),
      };
    }
